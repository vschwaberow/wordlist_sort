...
- **`index` / `query`**: subcommands for index-first UX. `index <out> <in...>` implies sort+dedup and writes cdb/fst/pthash (format from `--format` or `.fst`/`.cdb`/`.pthash`/`.wlp`). `query [-o out] <index> <queries...>` loads the index like `--lookup` (default out=`-`); `--miss` emits non-hits; mutually exclusive with `--exclude`/`--intersect`/`--lookup`.
...
- **`--rules FILE` / `--ruleset basic` / `--rules-max N`**: hashcat-style rule expansion after transforms, before membership (opcode subset; default max 64 variants/word incl. original).
- **`--bloom` / `--no-bloom` / `--bloom-bits N`**: Bloom early-drop gate before exact membership. Auto-on when B is built from text; off for prebuilt `.cdb`/WLTRIE1/WLPTH1 unless `--bloom`. `--no-bloom` disables. Bits/key default 10 (clamp 4–24).
