...
- **Compressed text output**: writing `--format=text` to a `.gz` / `.zst` (`.zstd`) / `.xz` path compresses with zlib / libzstd / liblzma. `--append` adds another member/frame/stream. Binary formats reject compressed outputs.
