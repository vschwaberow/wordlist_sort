...
- **Compressed text output**: writing `--format=text` to a `.gz` / `.zst` (`.zstd`) path compresses with zlib / libzstd. `--append` adds another gzip member or zstd frame. Binary formats reject compressed outputs.
